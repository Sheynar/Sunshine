...
namespace video {
  ...
  std::unique_ptr<avcodec_encode_session_t> make_avcodec_encode_session(
    ...
    if (config.chromaSamplingType == 1) {
      ...
      if (config.dynamicRange && !video_format[encoder_t::DYNAMIC_RANGE_YUV444]) {
        BOOST_LOG(error) << video_format.name << ": YUV 4:4:4 HDR not supported"sv;
        return nullptr;
      }
  ...
  int probe_encoders() {
    ...
    auto adjust_encoder_constraints = [&](encoder_t *encoder) {
      ...
      if (active_av1_mode == 3 && !encoder->av1[encoder_t::DYNAMIC_RANGE] && !encoder->av1[encoder_t::DYNAMIC_RANGE_YUV444]) {
