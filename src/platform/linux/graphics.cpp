...
namespace egl {
  ...
  std::optional<sws_t> sws_t::make_yuv444(int in_width, int in_height, int out_width, int out_height, gl::tex_t &&tex) {
...

    {
      ...
      program = gl::program_t::link(compiled_sources[3].left(), compiled_sources[0].left());
      if (program.has_right()) {
        BOOST_LOG(error) << "GL linker: "sv << program.right();
        return std::nullopt;
      }
      ...
      program = gl::program_t::link(compiled_sources[3].left(), compiled_sources[1].left());
      if (program.has_right()) {
        BOOST_LOG(error) << "GL linker: "sv << program.right();
        return std::nullopt;
      }
    ...
    sws.cursor_framebuffer = gl::frame_buf_t::make(1);
    sws.cursor_framebuffer.bind(&sws.tex[0], &sws.tex[2]);
  ...
  int sws_t::blank(gl::frame_buf_t &fb, int offsetX, int offsetY, int width, int height, AVPixelFormat format) {
    ...
    BOOST_LOG(info) <<" format of video ------- "sv << format;
  ...
  std::optional<sws_t> sws_t::make(int in_width, int in_height, int out_width, int out_height, AVPixelFormat format) {
    ...
    BOOST_LOG(info) << "pixel format for EGL frame: "sv << (int) format;
  ...
  void sws_t::load_yuv444_ram(platf::img_t &img) {
    ...
    gl::ctx.BindTexture(GL_TEXTURE_2D, loaded_texture);
    // Загружаем данные (ожидается упакованный YUV в RGBA формате)
